...
async function enrichVideos(videos) {
  ...
  // ✅ فلترة الفيديوهات (إزالة أي ID فاضي)
  ...
  const missing = validVideos.filter(v => !cache[v.id]);
  ...
  return videos.map(v => ({
    ...v,
    title: updated[v.id] || v.title || v.id
  }));
}

// ===== DATA =====
const DATA = {
  شائعة: DATA_RAW.شائعة
    .filter(v => v.id && v.id.trim() !== "") // ✅ منع ID فاضي
    .map(v => ({
      ...v,
      title: getTitleCache()[v.id] || "جاري التحميل..."
    })),
